...
pub fn relative_module_path(from: &str, to: &str) -> String {
    ...
    for component in &to_parts[common..] {
        if let Component::Normal(value) = component {
            result.push(value.to_str().unwrap_or_default());
        }
    }
    if result.is_empty() {
        "./".to_string()
...
mod tests {
    ...
    fn identifiers_are_checked() {
        assert!(is_safe_identifier("module_1"));
        assert!(!is_safe_identifier("1module"));
    }
    #[test]
    fn paths_are_relative() {
        assert_eq!(relative_module_path("./a/index.js", "./b.js"), "../b.js");
    }
}
